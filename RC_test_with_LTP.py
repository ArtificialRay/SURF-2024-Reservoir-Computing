...
# plot performance:
def plot_performance(epochs, Train_loss, cv_loss):
    """
    绘制训练损失和测试损失的数值关于epochs数的关系
    :param epochs:
    :param Train_loss:
    :param cv_loss:
    ...
    epochs_list_show = [i+1 for i in range(0, epochs, 2)]
...
plot_performance(epochs,train_loss,CV_loss)

prediction = model(testX)
print("prediction shape-", prediction.shape)
print(len(prediction))

# 因为一开始对数据进行了缩放，首先需要做一些逆缩放
# 预测的数据是1列，但是缩放前的数据有5列，必须改变形状再使用inverse_transform
prediction_copies_array = np.repeat(prediction.detach().numpy().reshape(-1,1),5,axis=1)
pred = scaler.inverse_transform(prediction_copies_array)[:,0] # 只需要第1列
